...
public class AutoFillTextBoxSkin<T> extends SkinBase<AutoFillTextBox<T>>
                ...
                cell.focusedProperty().addListener(new InvalidationListener() {
                    ...
                    public void invalidated(Observable ove) {
                        ...
                        if (cell.getItem() != null && cell.isFocused()) {
                            // here we are using 'temporaryTxt' as temporary
                            // saving text
                            // If temporaryTxt length is 0 then assign with
                            // current rawText()
                            String prev = null;

                            // first check ...(either texmporaryTxt is empty
                            // char or not)
                            ...
                            prev = temporaryTxt;
                            textbox.textProperty().removeListener(
                                    AutoFillTextBoxSkin.this);
                            // textbox.rawTextProperty().removeListener(AutoFillTextBoxSkin.this);
                            // textbox.setText(prev);
                            textbox.textProperty().setValue(
                                    cell.getItem().toString());
                            // textbox.rawTextProperty().setValue(cell.getItem().toString());
                            // textbox.rawTextProperty().addListener(AutoFillTextBoxSkin.this);
                            textbox.textProperty().addListener(
                                    AutoFillTextBoxSkin.this);
                            textbox.selectRange(prev.length(), cell.getItem()
                                    .toString().length());
                            System.out.println(temporaryTxt.length() + "="
    ...
    /**
     * ************************************************************** This is
     * recursive function which gives the sum of X and Y position of all it's
     * Parent and give the exact position where Popup needs to be visibled.
     *
     * @param p
     *            javafx.scene.Parent
     * @param w
     ...
     *            double
     * @return <a href=
     *         "http://download.oracle.com/javafx/2.0/api/javafx/geometry/Dimension2D.html"
     *         >Dimension2D</a>
     **************************************************************** */
    /*
	 * public Dimension2D getDimension(Parent p, double w, double h){
	 * if(getScene().getRoot() == p) return new Dimension2D(w,h); else{ return
	 * getDimension
	 * (p.getParent(),w+p.getLayoutX()+p.getTranslateX(),h+p.getLayoutY
	 * ()+p.getTranslateY()); } }
	 */
    // @Override
	/*
	 ...
	 * if(ov.getValue().toString().length()<=0){ temporaryTxt = "";
	 * if(autofillTextbox.getFilterMode()){ * //listview.getItems().clear();
	 * //listview.getItems().addAll(data); listview.setItems(data); showPopup();
	 * 
	 * } else{ hidePopup(); } } }
	 */
    ...
    /**
     * ******************************************************** Selects the
     * current Selected Item from the list and the content of that selected Item
     * is set to textbox.
     * ********************************************************
     */
    public void selectList() {
        ...
        if (i != null) {
            textbox.setText(listview.getSelectionModel().getSelectedItem()
                    .toString());
            listview.getItems().clear();
    ...
    /**
     * **************************************************** This is the main
     * event handler which handles all the event of the listview and textbox
     *
     * @param evt ****************************************************
     */
    @Override
	...
	/*
	 * protected void layoutChildren() { double width = this.getWidth(); double
	 * height = this.getHeight(); textbox.resize(width, height);
	 * positionInArea(textbox, 0.0, 0.0, width, height, 0.0d, HPos.CENTER,
	 * VPos.CENTER);
	 * 
	 * }
	 */
    ...
    public void showPopup() {
        ...
        // Dimension2D dimen =
        // getDimension(getParent(),getLayoutX()+getTranslateX(),getLayoutY()+getHeight()
        // +getTranslateY());
        // SHOWING THE POPUP JUST BELOW TEXTBOX
        popup.show(getWindow(),
                getWindow().getX() + textbox.localToScene(0, 0).getX()
                        + textbox.getScene().getX(), getWindow().getY()
                        + textbox.localToScene(0, 0).getY()
                        + textbox.getScene().getY() + TITLE_HEIGHT);
